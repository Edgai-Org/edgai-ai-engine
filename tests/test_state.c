...
int main(void)
{
    ...
    static const struct {
        EdgaiIntent          intent;
        EdgaiSequenceState  expected;
        const char             *label;
    } TRANSITIONS[] = {
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_HOOK,     "CONCEPT  + ADVANCE → HOOK"     },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_PREDICT,  "HOOK     + ADVANCE → PREDICT"  },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_STEPS,    "PREDICT  + ADVANCE → STEPS"    },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_STEPS,    "STEPS+0  + ADVANCE → STEPS+1"  },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_STEPS,    "STEPS+1  + ADVANCE → STEPS+2"  },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_VERIFY,   "STEPS+2  + ADVANCE → VERIFY"   },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_PRACTICE, "VERIFY   + ADVANCE → PRACTICE" },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_CLOSE,    "PRACTICE + ADVANCE → CLOSE"    },
        { EDGAI_INTENT_ADVANCE, EDGAI_STATE_DONE,     "CLOSE    + ADVANCE → DONE"     },
    };

    int n = (int)(sizeof(TRANSITIONS) / sizeof(TRANSITIONS[0]));
    for (int i = 0; i < n; i++) {
        EdgaiSequenceState got = edgai_state_next(session, TRANSITIONS[i].intent);
