...
static const eduos_tier_config_t OS_TIERS[] = {
...
static const eduos_tier_config_t MOBILE_TIERS[] = {
...
const eduos_tier_config_t *eduos_tier_select(long long ram_kb, bool is_mobile)
{
     ...
     * when EDUOS_MODEL_OVERRIDE is set. */
    const eduos_tier_config_t *base;
    if (is_mobile) {
    ...
    /* EDUOS_MODEL_OVERRIDE — developer escape hatch.
     * Keeps RAM-tier context/batch/thread settings but loads the specified file. */
    const char *override = getenv("EDUOS_MODEL_OVERRIDE");
    if (override && override[0] != '\0') {
        static eduos_tier_config_t override_tier;
        override_tier              = *base;       /* copy all numeric fields */
        override_tier.model_filename = override;  /* swap in the override name */
        fprintf(stderr, "eduos: EDUOS_MODEL_OVERRIDE active — using '%s'\n", override);
